namespace Mammatus.Compute.DataStructures
{
    ...
    public sealed class Heap<T> where T : IComparable<T>
    {
        ...
        {
            if (ix == 0)
                throw new InvalidOperationException(Errors.ParentOfRootDoesntExist);
            else
            {
        ...
        public T Remove()
        {
            ...
            T result = default(T);
            if (lastIx < 0)
                throw new InvalidOperationException(Errors.CannotRemoveFromEmptyHeap);
            else
            {
                result = this.List[0];
        ...
        public T Peek()
        {
            if (this.List.Count > 0)
                ...
                throw new InvalidOperationException(Errors.PeekEmptyTree);
        }
